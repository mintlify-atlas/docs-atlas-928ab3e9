...
<Steps>
  ...
  <Step title="llama-server startup">
    The orchestrator spawns `llama-server` with `--models-dir models/ --models-max 1 --parallel <N> --ctx-size <N> -ngl <ngl>`. GPU-accelerated systems receive `-ngl 99` to offload all layers; CPU-only systems receive `-ngl 0`. llama-server binds to port **10086**.
  </Step>
...
1. **Available memory** — reads free VRAM from `nvidia-smi` for CUDA, uses total RAM minus overhead for CPU.
