...
<ParamField query="ODYSSEUS_BACKEND" type="string">
  Selects the inference backend. Accepted values are `llama` (or its aliases `llamacpp` and `llama.cpp`) and `ollama`. When not set, the value from `data/launcher_config.json` is used; if that is also absent, the orchestrator prompts interactively and defaults to `llama`.
</ParamField>
...
<ParamField query="ODYSSEUS_ALLOW_SYSTEM_GIT" type="string">
  Unix-only. Set to `1` to allow the orchestrator to use the host system's `git` executable instead of downloading a portable Git binary. On Linux and macOS the launcher already falls back to system `git` automatically when a portable binary is not present, but this flag makes the preference explicit.
</ParamField>
