...
<ParamField body="backend" type="string">
  Inference backend to use on every launch. Accepted values: `"llama"` or `"ollama"`. When this field is present and valid, the interactive backend-selection prompt is skipped entirely.
</ParamField>
...
<Tip>
  When you select a backend from the interactive prompt at startup, that choice is automatically written back to `data/launcher_config.json`. This means the prompt only appears once — on every subsequent launch the saved backend is used directly, skipping the question entirely.
</Tip>
