...
<Tabs>
  <Tab title="Windows">
    <Steps>
      ...
      <Step title="Wait for the first-run setup to complete">
        On first launch, the orchestrator downloads portable Node.js, a Python 3.12 embedded environment, and a CUDA/Vulkan/CPU-optimized `llama-server.exe` binary — all into the `bin\` folder. A model selection prompt appears once downloads finish.
      </Step>
      <Step title="Select a model and open the UI">
        Choose a model from the prompt, then navigate to `http://127.0.0.1:7070` in your browser. The orchestrator opens it automatically if your default browser is configured.
  ...
  <Tab title="macOS">
    <Steps>
      ...
      <Step title="Select a model and open the UI">
        On first run, select your model when prompted. The orchestrator then opens `http://127.0.0.1:7070` in your default browser automatically.
  ...
  <Tab title="Linux">
    <Steps>
      ...
      <Step title="Select a model and open the UI">
        Choose a model from the prompt on first launch. The orchestrator opens `http://127.0.0.1:7070` via `xdg-open` automatically.
...
On first launch — or when no saved preference exists — you will be prompted to choose an inference backend:

```text
Choose inference backend:
  [1] Ollama    - webapp-managed downloads and model switching
  [2] llama.cpp - portable GGUF llama-server (default)

Enter selection [1-2] (default 2 - llama.cpp):
```
