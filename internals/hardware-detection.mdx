...
<Note>
  `gpuMemoryGB` and `gpuFreeMemoryGB` are only populated for CUDA GPUs via `nvidia-smi`. Metal and Vulkan detection does not query VRAM, so those fields remain `0` for Apple Silicon and Vulkan devices. Context window sizing for those backends falls back to system RAM instead.
</Note>
...
**GPU layer offload (`ngl`)** — controls how many model layers are offloaded to the GPU. A CPU-only machine gets `ngl = 0`; any GPU backend gets `ngl = 99` (all layers):

```javascript
const ngl = hw.gpuBackend === 'cpu' ? 0 : 99;
```

**Context window size** — the llama.cpp backend auto-calculates a safe context size based on available memory. The priority for the memory figure used in this calculation is:

1. `hw.gpuFreeMemoryGB` — free VRAM (CUDA only)
2. `hw.gpuMemoryGB` — total VRAM if free VRAM is unavailable
3. `hw.ramGB` — system RAM for CPU inference
